...
print(f"[✓] Open ID: {NEW_OPEN_ID}")
print(f"[✓] Platform: {platform_}")
...
try:
    ...
    print(f"[INFO] MajorLogin HTTP status: {response.status_code}")
except Exception as e:
    print("[!] MajorLogin request failed:", e)
    sys.exit(1)
if not response.ok:
    print("[!] MajorLogin returned error code:", response.status_code)
    sys.exit(1)
...
try:
    ...
    print("[✓] Parsed MajorLoginRes from decrypted payload")
except Exception:
    resp_msg.ParseFromString(resp_enc)
    parsed_data = SimpleProtobuf.parse_protobuf(resp_enc)
print(f"[✓] Account ID: {resp_msg.account_id}")
print(f"[✓] JWT: {resp_msg.account_jwt[:50]}...")
print(f"[✓] Key: {resp_msg.key.hex()}")
print(f"[✓] IV: {resp_msg.iv.hex()}")
field_21_value = parsed_data.get(21, None)
if field_21_value:
    print(f"[✓] Field 21 extracted: {field_21_value}")
    ts = Timestamp()
    ...
    timetamp = ts.seconds * 1_000_000_000 + ts.nanos
print(f"[✓] Calculated timetamp: {timetamp}")
...
try:
    ...
    print(f"[INFO] GetLoginData HTTP status: {r2.status_code}")
    r2.raise_for_status()
except Exception as e:
    print("[!] GetLoginData request failed:", e)
    sys.exit(1)
online_ip = None
online_port = None
    ...
    try:
        ...
        if json_result:
            ...
            if '14' in parsed_data_login and 'data' in parsed_data_login['14']:
                ...
                print("[!] Could not find field 14 in parsed data")
                sys.exit(1)
        else:
            print("[!] Failed to parse GetLoginData response")
            sys.exit(1)
    except Exception as e:
        print(f"[!] Error processing GetLoginData response: {e}")
        traceback.print_exc()
        sys.exit(1)
else:
    print("[!] GetLoginData returned error:", r2.status_code)
    sys.exit(1)
...
if not final_token_hex:
    ...
    sys.exit(1)
print(f"[✓] Packet built successfully")
